  ...
  <div class="app-container" style="background-color: #f4f7f9; padding: 24px; min-height: 100vh;">
    ...
    <el-card shadow="never" class="table-card">
      ...
      >
            ...
            <div class="expand-container">
              ...
              >
                ...
                <el-table-column label="配送方式" min-width="100" align="center">
                  <template slot-scope="item">
                    <el-tag v-if="item.row.isSelfPick" size="mini" type="success" effect="plain">仓库自提</el-tag>
                ...
                <el-table-column label="收货人" min-width="100" align="center" >
                  <template slot-scope="item">
                    {{ item.row.isSelfPick ? '--' : (item.row.deliveryInfo.receiverName || '--') }}
                  </template>
                </el-table-column>
        ...
        <el-table-column label="管理" fixed="right" align="center" width="180">
          <template slot-scope="scope">
            <el-dropdown @command="(status) => handleUpdateStatus(scope.row, status)" trigger="click">
              <el-button type="text" size="small">变更状态<i class="el-icon-arrow-down el-icon--right"></i></el-button>
              <el-dropdown-menu slot="dropdown">
                <template v-if="scope.row.orderStatus === '已关闭' || scope.row.orderStatus === '草稿'">
                  <el-dropdown-item command="待发货">待发货</el-dropdown-item>
                  <el-dropdown-item command="部分发货">部分发货</el-dropdown-item>
                  <el-dropdown-item command="已完成">已完成</el-dropdown-item>
                </template>
                <template v-else-if="scope.row.orderStatus !== '已完成'">
                  <el-dropdown-item command="已关闭">关闭订单</el-dropdown-item>
                </template>
              </el-dropdown-menu>
            </el-dropdown>
...
<script>
import { getOrderList, updateOrderStatus, delOrder } from "@/api/wx/order";
...
export default {
  ...
  methods: {
    ...
    getDeliveryStatusColor(status) {
      const colorMap = {
        '草稿': '#909399',   // 对应 index.wxss 的灰色
        '待发货': '#E6A23C', // 对应 index.wxss 的黄色
        '已发货': '#409EFF', // 对应 index.wxss 的蓝色/部分发货色
        '已收货': '#67C23A'  // 对应 index.wxss 的绿色
      };
    ...
    handleUpdateStatus(row, status) {
      this.$confirm(`确认将订单 ${row.orderNo} 状态修改为【${status}】吗？`, "提示", {
    ...
    handleDelete(row) {
      this.$confirm(`是否确认删除编号为 "${row.orderNo}" 的订单数据？删除后无法恢复！`, "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "error"
      }).then(() => {
    ...
    getStatusTag(s) {
      const map = { '待发货': 'warning', '已完成': 'success', '已关闭': 'danger', '部分发货': 'primary', '草稿': 'info' };
      return map[s] || 'info';
    }
  }
};
